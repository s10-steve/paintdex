 ...
 *
 * Ordered name A–Z within brand — the same order the build scripts
 * (`build-browse-index.ts`, `build-similar-index.ts`) rank in. Keeping the
 * orders identical means `findSimilar` over this list breaks ties (paints at an
 * equal CIEDE2000 distance) the same way the precomputed similar-index does, so
 * the two stay in exact parity.
 */
...
function getSimilarIndex(): SimilarIndex {
  if (!similarCache) {
    ...
    // (dynamicParams = false), so this never runs at request time in prod.
    const path = join(process.cwd(), ".cache", "similar-index.json");
    try {
      similarCache = JSON.parse(readFileSync(path, "utf8")) as SimilarIndex;
    } catch (err) {
      throw new Error(
        `Could not read the similar-colour index at ${path}. ` +
          "Run `npm run build:similar-index` first (it runs automatically via " +
          "prebuild/predev).",
        { cause: err },
      );
    }
